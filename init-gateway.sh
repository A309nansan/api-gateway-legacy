...
JWT_SECRET_KEY=$(echo "$SECRET_RESPONSE" | jq -r '.data.data.token.external')
...
# 기존 서비스 삭제
log "gateway undeploy"
docker rm -f gateway

# 기존 gateway 이미지를 삭제하고 새로 빌드
log "gateway image remove and build."
docker rmi gateway:latest || true
docker build -t gateway:latest .

# Docker로 gateway 서비스 실행
log "Execute gateway..."
docker run -d \
  --name gateway \
